<p align="center">

<a href="https://runer0101.github.io" style="text-decoration:none;">
  <div style="
    width:380px;
    background-color:#0d1117;
    border:1px solid #00ff88;
    border-radius:8px;
    padding:24px;
    text-align:center;
    font-family:-apple-system,BlinkMacSystemFont,'Segoe UI','Noto Sans',Helvetica,Arial,sans-serif;
    box-shadow: 0 4px 24px rgba(0, 255, 136, 0.15);
  ">
    <div style="display:flex;align-items:center;justify-content:center;margin-bottom:6px;">
      <svg style="margin-right:10px;" width="20" height="20" viewBox="0 0 16 16" fill="#00ff88">
        <path fill-rule="evenodd" d="M2 2.5A2.5 2.5 0 014.5 0h8.75a.75.75 0 01.75.75v12.5a.75.75 0 01-.75.75h-2.5a.75.75 0 110-1.5h1.75v-2h-8a1 1 0 00-.714 1.7.75.75 0 01-1.072 1.05A2.495 2.495 0 012 11.5v-9zm10.5-1V9h-8c-.356 0-.694.074-1 .208V2.5a1 1 0 011-1h8zM5 12.25v3.25a.25.25 0 00.4.2l1.45-1.087a.25.25 0 01.3 0L8.6 15.7a.25.25 0 00.4-.2v-3.25a.25.25 0 00-.25-.25h-3.5a.25.25 0 00-.25.25z"/>
      </svg>
      <span style="color:#00ff88;font-size:22px;font-weight:700;">Mi Portafolio Web</span>
    </div>
    <p style="color:#7ee787;font-size:12px;margin:0;">Click para ver mis proyectos</p>
  </div>
</a>

</p>

<br/>

<p align="center">

<a href="https://zentramid.eu.cc" style="text-decoration:none;">
  <div style="
    width:380px;
    background:linear-gradient(135deg, #1a1b26 0%, #1f2335 100%);
    border:1px solid #f7768e;
    border-radius:8px;
    padding:20px;
    text-align:left;
    font-family:-apple-system,BlinkMacSystemFont,'Segoe UI','Noto Sans',Helvetica,Arial,sans-serif;
    box-shadow: 0 4px 20px rgba(247, 118, 142, 0.15);
  ">
    <div style="display:flex;align-items:center;margin-bottom:10px;">
      <svg style="margin-right:8px;" width="16" height="16" viewBox="0 0 16 16" fill="#f7768e">
        <path fill-rule="evenodd" d="M2 2.5A2.5 2.5 0 014.5 0h8.75a.75.75 0 01.75.75v12.5a.75.75 0 01-.75.75h-2.5a.75.75 0 110-1.5h1.75v-2h-8a1 1 0 00-.714 1.7.75.75 0 01-1.072 1.05A2.495 2.495 0 012 11.5v-9zm10.5-1V9h-8c-.356 0-.694.074-1 .208V2.5a1 1 0 011-1h8zM5 12.25v3.25a.25.25 0 00.4.2l1.45-1.087a.25.25 0 01.3 0L8.6 15.7a.25.25 0 00.4-.2v-3.25a.25.25 0 00-.25-.25h-3.5a.25.25 0 00-.25.25z"/>
      </svg>
      <span style="color:#7aa2f7;font-size:18px;font-weight:700;">Zentramind</span>
      <span style="
        background-color:#f7768e;
        color:#1a1b26;
        font-size:10px;
        font-weight:700;
        border-radius:24px;
        padding:2px 10px;
        margin-left:12px;
      ">BETA</span>
    </div>
    <p style="color:#a9b1d6;font-size:13px;margin:0 0 14px 0;line-height:1.6;">
      AI-powered WhatsApp CRM — lead scoring, auto-replies, and live conversation takeover. Multi-tenant SaaS with real clients in production.
    </p>
    <div style="display:flex;align-items:center;gap:14px;font-size:12px;color:#8b949e;">
      <span style="display:flex;align-items:center;gap:4px;">
        <span style="width:10px;height:10px;border-radius:50%;background-color:#9ece6a;"></span>
        Next.js
      </span>
      <span style="display:flex;align-items:center;gap:4px;">
        <span style="width:10px;height:10px;border-radius:50%;background-color:#bb9af7;"></span>
        Fastify
      </span>
      <span style="display:flex;align-items:center;gap:4px;">
        <span style="width:10px;height:10px;border-radius:50%;background-color:#7aa2f7;"></span>
        PostgreSQL
      </span>
    </div>
  </div>
</a>

</p>